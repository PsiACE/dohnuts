...
class RLCDTests(unittest.TestCase):
    def test_loss_reward_and_gradient_match_pinned_laya(self):
        """Golden values were evaluated by the actual upstream Python functions."""
        fixture = json.loads((Path(__file__).parent / "fixtures/laya-rlcd.json").read_text())
        for case in fixture["cases"]:
            ...
            torch.testing.assert_close(
                loss.detach(), torch.tensor(case["loss"]), atol=1e-6, rtol=1e-5
            )
            torch.testing.assert_close(metrics["reward_mean"], torch.tensor(case["reward_mean"]))
            torch.testing.assert_close(
                logits.grad, torch.tensor(case["gradient"]), atol=1e-6, rtol=1e-5
            )

    def test_ordinal_reward_respects_distance(self):
        config = RLCDConfig()
        ...
        self.assertGreater(rewards[1, 0].item(), rewards[0, 0].item())
